...
# Figure 4a
range_pl <- 
  ggplot(pl.mark, aes(x=Rank, y=log10(Mean), fill=Marker, color=Marker)) +
...
# Figure 4b
range_EV <- 
  ggplot(ev.mark, aes(x=Rank, y=log10(Mean), fill=Marker, color=Marker)) +
  geom_point(alpha=0.5) +
  scale_color_manual(values=c("EV"="blue", "Plasma"="red2", "none"="grey40")) + 
  ...
  scale_y_continuous(limits=c(2, 12.5)) +
  theme_bw() +
  theme(legend.position="none") #+ scale_x_continuous(limits=c(1,400))
range_EV
